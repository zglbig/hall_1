...
public class FriendsManager {
     ...
     * 添加好友
     */
    public Object addFriend(UserMap selfMap, long targetUid){
        ...
        UserMap targetMap = SessionManager.getSession(targetUid);
        if(targetMap == null){
            targetMap = selectUser(targetUid);
            if(targetMap == null)
                new GenaryAppError(AppErrorCode.FRIEND_NOT_USER_ERR);
        }
        if(frinedsModel == null){
            frinedsModel = new SQLFrinedsModel();
        }else if(frinedsModel.hasFriend(targetUid+""))
     ...
     * 删除好友
     */
    public Object removeFriend(UserMap um,long targetAccount){
        //更新字段
     ...
     * 赠送礼物
     */
    public Object sendGift(UserMap selfMap,long targetAccount,int giftId,int count){
    ...
    private void updateTask(UserMap userMap){
     ...
     * 好友追踪
     */
    public Object selectFriend(UserMap userMap,long targetAccount){
        return null;
    }
     ...
     */
    public Object addEnemy(UserMap self,long enemyAccount){
     ...
     * @param targetAccount
     */
    public Object removeEnemy(UserMap um,long targetAccount){
        if(um.getFriends().removeEnemy(targetAccount+""))
            updateUser(um);
        return null;
    }
    private UserMap selectUser(long uid){
        IUserDao userDao = SpringUtils.getBean(IUserDao.class);
        DBUser u = userDao.queryDBUserByUid(uid);
        if(u == null)
            new GenaryAppError(AppErrorCode.FRIEND_NOT_USER_ERR);
        return PlayerInit.initUserMap(u);
    }
    private void updateUser(UserMap um){
        DBUser user = new DBUser();
        user.setFriends(JsonUtils.jsonSerialize(um.getFriends()));
        user.setId(um.getId());
        um.update(user);
    }
}
